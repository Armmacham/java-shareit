...
class BookingController {
    ...
    public List<BookingDTO> getAllUserBookings(@RequestParam(value = "state", defaultValue = "ALL") String state,
        ...
        return bookingService.getAllBookingsOfCurrentUser(State.convert(state), userId, PageRequest.of(from / size, size));
    }
    ...
    public List<BookingDTO> getAllUserItemsBookings(@RequestParam(value = "state", defaultValue = "ALL") String state,
                                                    ...
                                                    @RequestParam(value = "from", defaultValue = "0") @Min(0) Integer from,
                                                    @RequestParam(value = "size", defaultValue = "10") @Min(1) @Max(20) Integer size) {
        return bookingService.getAllBookingsOfOwner(State.convert(state), userId, PageRequest.of(from / size, size));
    }
